...
namespace App\BoundedContext\VideoGamesRecords\Core\Application\DataProvider;
...
class TopScoreProvider
{
     ...
     * @throws ORMException
     */
    public function load(Group $group, string $locale = 'en'): mixed
    {
        $player = $this->getPlayer();
        $query = $this->em->createQueryBuilder()
            ->select('ch')
            ->from('App\BoundedContext\VideoGamesRecords\Core\Domain\Entity\Chart', 'ch')
            ->join('ch.group', 'gr')
            ->addSelect('gr')
            ->addSelect('pc')
