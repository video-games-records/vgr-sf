    ...
    public function configureActionButtons(array $buttonList, string $action, ?object $object = null): array
    {
        if (in_array($action, ['show', 'edit', 'acl']) && $object) {
            ...
            $buttonList['add-lib-chart'] = [
                'template' => '@VideoGamesRecordsCore/admin/action_button/btn.add_lib_chart.html.twig',
            ];
        ...
        if (($this->hasRequest()) && ($this->isCurrentRoute('create'))) {
            ...
            if ($this->getRequest()->getSession()->has('vgrcorebundle_admin_group.idGame')) {
                ...
                $entityManager = $this->getModelManager()
                    ->getEntityManager('App\BoundedContext\VideoGamesRecords\Core\Domain\Entity\Game');
                $game = $entityManager->getReference('App\BoundedContext\VideoGamesRecords\Core\Domain\Entity\Game', $idGame);
