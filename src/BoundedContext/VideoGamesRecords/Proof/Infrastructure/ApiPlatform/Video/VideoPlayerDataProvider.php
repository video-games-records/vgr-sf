...
class VideoPlayerDataProvider implements ProviderInterface
{
    ...
    {
        $playerId = (int) $uriVariables['playerId'];
        $page = (int) ($context['filters']['page'] ?? 1);
        $itemsPerPage = (int) ($context['filters']['itemsPerPage'] ?? $operation->getPaginationItemsPerPage() ?? 30);
        $offset = ($page - 1) * $itemsPerPage;

        $videos = $this->videoRepository->findActiveVideosByPlayerPaginated($playerId, $offset, $itemsPerPage);
        $totalItems = $this->videoRepository->countActiveVideosByPlayer($playerId);
