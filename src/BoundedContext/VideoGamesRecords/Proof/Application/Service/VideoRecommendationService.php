...
namespace App\BoundedContext\VideoGamesRecords\Proof\Application\Service;
...
class VideoRecommendationService
{
    ...
    private const string CACHE_PREFIX = 'video_recommendations_';
    ...
    private function generateRecommendations(Video $video, int $limit): array
    {
        $recommendations = [];
        $videoId = $video->getId();
        $usedVideoIds = $videoId !== null ? [$videoId] : [];
    ...
    private function extractVideoIds(array $videos): array
    {
        return array_values(array_filter(
            array_map(fn(Video $video) => $video->getId(), $videos),
            fn($id) => $id !== null
        ));
    }

    /**
     * @param array<Video> $candidateVideos
     * @return array<Video>
     */
     ...
     * @param array<Video> $rankedVideos
     * @return array<Video>
     */
