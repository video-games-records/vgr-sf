...
namespace App\BoundedContext\VideoGamesRecords\Badge\Presentation\Admin;
...
class PlayerBadgeAdmin extends BaseAdmin
{
    protected function generateBaseRouteName(bool $isChildAdmin = false): string
    {
        return 'vgrcorebundle_admin_player_badge';
    }
    ...
    public function configureQuery(ProxyQueryInterface $query): ProxyQueryInterface
    {
        ...
        $query->leftJoin($query->getRootAliases()[0] . '.player', 'p')
              ->leftJoin($query->getRootAliases()[0] . '.badge', 'b')
              ->addSelect('p', 'b');
        return $query;
    }
